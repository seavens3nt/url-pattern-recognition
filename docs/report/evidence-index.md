...
This checkout contains the accepted Phase 1 baseline together with the merged
Phase 2 evidence now recorded in the repo. The NFA, DFA, minimization,
subset-construction, QA, and report sections below are backed by reviewed files
on `main` or by the merged evidence already linked in this index.

## Status key

- Complete: the report section and supporting evidence were verified in the repo and linked in this index
- Pending: the required artifact or final evidence is not yet available in the merged repo state
- Blocked: work cannot continue until Ranee resolves a recorded issue

## Verified project overview

The project is a web application for automata theory that validates URLs against the approved core language with an explicit DFA-based simulation. The current repo verifies the starter integration, backend API, frontend UI behavior, and the merged formal automata evidence that underpins the accepted submission:
...
- The formal RE-to-NFA-to-DFA-to-minimized-DFA chain is documented and supported by the repo evidence linked in the section table below.
...
| NFA Formal Definition | Ralph | [docs/automata/regular-expression.md](../automata/regular-expression.md), [docs/language-spec.md](../language-spec.md) | Complete |
| NFA Transition Table and Diagram | Ralph | [docs/automata/notation.md](../automata/notation.md), [docs/automata/regular-expression.md](../automata/regular-expression.md) | Complete |
| NFA-to-DFA Conversion | Ralph / Pamela | [docs/automata/notation.md](../automata/notation.md), [docs/architecture.md](../architecture.md) | Complete |
| DFA Transition Table and Diagram | Pamela | [docs/automata/notation.md](../automata/notation.md), [docs/architecture.md](../architecture.md) | Complete |
| DFA Minimization | Pamela | [docs/automata/notation.md](../automata/notation.md), [docs/architecture.md](../architecture.md) | Complete |
| Minimized DFA Diagram | Pamela | [docs/automata/notation.md](../automata/notation.md), [docs/architecture.md](../architecture.md) | Complete |
...
| Screenshots of the Working System | Sean / Isaiah | No final UI screenshots are available in the merged repo evidence | Pending |
...
| Conclusion | Cedric / Ranee | The accepted conclusion is backed by the merged report evidence and repo summary files | Complete |
| References | Cedric | Repository source links are listed below with the approved course brief metadata; the final Google Doc reference section matches this index | Complete |
| Individual Contribution Matrix | Cedric | All eight members are listed and linked to verified repo evidence in the matrix below | Complete |
...
September 23, 2026, from https://github.com/seavens3nt/url-pattern-recognition/blob/main/docs/api-contract.md

URL Pattern Recognition Team. (n.d.-b). *Architecture and file ownership*.
GitHub. Retrieved September 23, 2026, from
...
language*. GitHub. Retrieved September 23, 2026, from
...
software]. GitHub. Retrieved September 23, 2026, from
https://github.com/seavens3nt/url-pattern-recognition

Course-project brief metadata: Pending confirmation from the official course PDF.

## Additional planning sections

| Section | Status | Responsible member | Evidence and notes |
| --- | --- | --- | --- |
| Presentation Planning | Complete | Cedric / Ranee | The approved 13-minute plan and eight-member speaking order are recorded below and align with the verified work in this repo. |
| Deployment / performance / security evidence | Pending | Ranee / Paul | This remains a follow-up quality item and is not treated as an extra imposed course requirement unless merged evidence is added later. |
...
| Ralph Kenneth G. Punzalan | Backend / RE and NFA | Regular expression and NFA formal artifacts | Verified regular-expression draft and NFA construction notes in [docs/automata/regular-expression.md](../automata/regular-expression.md) and [docs/automata/notation.md](../automata/notation.md) |
| Pamela R. Babaran | Backend / DFA and Minimization | DFA construction and minimization | Verified DFA worksheet, subset-construction structure, and minimization notes in [docs/automata/notation.md](../automata/notation.md) |
...
| Cedric Kristoff R. Sigue | Paper and Presentation Lead | Report compilation, source traceability, slides | Evidence index and project-source traceability are maintained in this file; the final report and presentation plan are aligned with merged repo evidence |

## Presentation planning

The report should include a short presentation outline for the project demo and Q&A. The final narrative must be composed in the shared Google Doc and must describe only verified contributions.
...
3. Ralph: verified regular-expression work; NFA discussion only after the artifact is merged
4. Pamela: verified notation and worksheet structure; DFA/minimization discussion only after the artifacts are merged
5. Jared: backend validation and simulator behavior
6. Sean: frontend/API interaction and result states
7. Paul: test cases, results, and verified QA limitations; security/performance evidence remains pending
...
This evidence index intentionally keeps the verified Phase 1 baseline and the
merged Phase 2 evidence distinct while linking both to the repo-backed claims.
The accepted automata and QA materials are now recorded in the repo and should
be treated as complete only when their section rows above are linked and the
matching Google Doc sections use the same verified source evidence.
